...
#include "..\..\valve_sdk\interfaces\IClientUnknown.hpp"
...
std::array<Vector, 3> PizdaDrawList::Add3DCube(C_BasePlayer* pEntity, PizdaColor c1, bool b_outline, bool filled, PizdaColor fill_col)
{
	if (!pEntity)return std::array<Vector, 3>();
	auto coll = pEntity->GetCollideable();
	...
	float ang = pEntity->m_angEyeAngles().pitch;
	...
	int j = 0;
	for (int i = 0; i <= 7; i++)
		if (!Math::WorldToScreen(pEntity->GetRenderOrigin() + corners[i], _corners[i]))return array<Vector, 3>();
	for (int i = 1; i <= 4; i++)
	{
		if (!b_outline) {
...
float PizdaDrawList::AddText(Vector2D start, bool center, bool shadow, PizdaColor color, string fontname, const char* format, ...) {
	...
	//vsprintf_s(Buffer, format, va_alist);
	va_end(va_alist);
	return AddText_NF(start, center, shadow, color, fontname, Utils::utf8toWStr(Buffer).c_str());
}
...
float PizdaDrawList::AddText(Vector2D start, bool center, bool shadow, PizdaColor color, string fontname, float scale, const char* format, ...) {
	...
	//vsprintf_s(Buffer, format, va_alist);
	va_end(va_alist);
	return AddText_NF(start, center, shadow, color, fontname, scale, Utils::utf8toWStr(Buffer).c_str());
}
